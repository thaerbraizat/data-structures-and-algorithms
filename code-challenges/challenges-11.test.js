...
------------------------------------------------------------------------------------------------ */
function transformToLis(o){
  let list = Object.keys(o).map((item)=>{
    return `<li>${item}: ${o[item]}</li>`
  });
  return list;
}
/* ------------------------------------------------------------------------------------------------
...
const totalSum = (input) => {
  ...
  for(let i=0; i< input.length ; i++){
    let innerSum=input[i].reduce((acc,num)=>{
     acc=acc+num;
     return acc;
    },0)
    sum = sum+innerSum;
...
let findShortest = (data) => {
  let test= data.sort((a,b)=>{
    let c =parseInt(a.height);
    let d= parseInt(b.height);
    return c-d;
  })
  let theTest = test[0].name;
  return theTest;
};
/* ------------------------------------------------------------------------------------------------
TESTS
