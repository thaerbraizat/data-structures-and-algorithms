...
const maxInArray = (arr) => {
  let maxValue = arr.reduce((x, y) => Math.max(x, y))
  return maxValue;
};
...
Write a function named checkValues that take
s in an object and a value and returns true if the value is in the object.
...
const updateNumbers = (obj) => {
  let join=Object.entries(obj).map((arr)=>{
  return arr.join(": ");
});
return join;
};
