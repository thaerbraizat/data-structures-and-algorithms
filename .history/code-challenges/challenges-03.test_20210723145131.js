...
const containsAnd = (arr) => {
  // Solution code here...
  let array=arr.filter(element=>  element.includes("and"));
  return(array);
};
...
const oddValues = (arr) => {
  // Solution code here...
  let array=arr.filter(element=>  element % 2 !== 0);
  return(array);
};
...
Write a function named notInFirstArray that,
 given two arrays as input,
  uses filter to return an array of all the elements in
   the second array that are not included in the first array.
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let array=arr.filter(element=>  !forbiddenValues.includes(element));
  return(array);
};
