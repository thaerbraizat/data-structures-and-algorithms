...
Write a function named screenForNames that takes in an array of strings and uses Regex to creat
e a new array of only those strings that match the following rules:
...
const screenForNames = (arr) => {
  const re = /(Mr\.|Mrs\.|Ms\.|Dr\.)\ (\w+|\w+ \w+)/g;
  const re2 = /^ +/;
  let arr2 = [];
  let arr3 = [];

  arr.map((name) => {
    if (name.match(re) !== null) arr2.push(name);
  });

  arr2.map((name) => {
    if (name.match(re2) === null) arr3.push(name);
  });
...
const isSecure = (url) => {
    ...
    return url.match(/^https:\/\//)[0] === 'https://';
    }
  };
